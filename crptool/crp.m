...
%
% Revision 5.19  2016/03/03 14:57:40  marwan
...
%
% Revision 5.18  2015/07/09 16:15:25  marwan
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% computation
  ...
  % use builtin implementation
      ...
      %%%%%%%%%%%%%%%%% maximum norm, fixed RR
          errcode=115;
          s = max(abs(s1),[],3);
          % s = sqrt(sum(s1.^2, 3)); % using euclidean distance instead of maximum distance
          ss = sort(s(:));
          idx = ceil(e * (length(ss)+NX)); % +NX is necessary to remove the diagonal line
          e = ss(idx);
