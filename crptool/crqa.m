...
%
% Revision 5.58  2016/09/23 12:55:27  marwan
...
%
% Revision 5.55  2016/03/04 14:37:15  marwan
% bug in using time crqa(x,m,tau,.1,'fan','sil','non')series without explicit time scale fixed
%
% Revision 5.54  2016/03/03 14:57:40  marwan
...
%
% Revision 5.53  2016/03/02 16:53:21  marwan
% bug in time axis fixed
%
% Revision 5.52  2016/02/05 17:16:37  marwan
% fix compatibility issues (R2014b)
%
% Revision 5.51  2015/06/20 16:55:05  marwan
...
errcode=1;
   ...
   time_scale_flag=0; time_scale_flag_x=0;
   nogui=0;
   ...
   if nargin & ischar(varargin{1})
        ...
        x=temp{1}; y=temp{2}; time_scale_flag=temp{3}; xscale=temp{4};
  ...
  %      if sum(double(diff(x(:,1))<=0)), time_scale_flag=0; end
   ...
   % check if a time scale is available   
   if isempty(xscale)
...
case 'init'
  ...
  h8=figure(props.window,...                                % Plot Figure
            ...
            'MenuBar','none', ...
            'UserData',{x,y,time_scale_flag,xscale},...
  ...
  h2=textwrap(h,{[char(169),' AGNLD'],'University of Potsdam','1998-2016'});
  set(h,'String',h2)
...
%%%%%%%%%%% plots
  ...
  ylabel('Variance')
  if undocumented, ylabel('max Rec Time'); end
...
if ~nogui
    ...
    if undocumented, tx{9} = 'max Rec Time'; end
