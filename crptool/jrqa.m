...
%
% Revision 2.10  2016/02/05 17:16:37  marwan
% fix compatibility issues (R2014b)
%
% Revision 2.9  2014/09/23 07:11:25  marwan
% Matlab R2014b incompatibility issues fixed
%
% Revision 2.8  2013/08/22 06:37:50  marwan
...
if nogui~=2 & ishandle(hw), waitbar(1); drawnow; close(hw), end
  ...
  if ~nogui
    set(h_button_apply(1),'ToolTip','Starts the computation.','String','Apply','Callback','jrqa compute')
    for j=1:length(obj); 
        h=findobj('Tag',obj{j},'Parent',h_fig(1)); 
        if ~isempty(h)
          set(h,'Enable','On')
